...
test("authenticates, mutates files, and restores from trash", async () => {
  ...
  try {
    ...
    const projectPayload = projectListing.json() as { items: Array<{ path: string }> };
    ...
    const restoredPayload = restoredListing.json() as { items: Array<{ path: string }> };
...
test("streams previews, supports ranges, and manages trash deletion", async () => {
  ...
  try {
    ...
    const trashPayload = trashResponse.json() as { items: Array<{ id: string }> };
