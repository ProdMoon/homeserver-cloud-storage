...
async function sendFileStream(
  ...
  reply.header("Content-Disposition", `${disposition}; filename*=UTF-8''${encodeFilename(entry.name)}`);
...
export async function createApp(options: AppOptions = {}) {
  ...
  app.post("/api/files/upload", async (request) => {
    ...
    for await (const part of parts) {
      ...
      uploaded.push(await resolveEntry(config.rootDir, path.posix.join(destination.relativePath, safeName)));
    }
